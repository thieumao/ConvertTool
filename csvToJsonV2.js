...
const writeJsonFile = async () => {
  const csv = await readFileAsync('./mobile.csv', 'utf8');
  ...
  for (const index in arr) {
    ...
    const viValue = lineArr.length >= 2 ? lineArr[2].replace('"',"").replace('"',"") : ''; // get vietnamese columns
    let keyArr = key.replace('"',"").replace('"',"").split('/');
    _.set(viJson, keyArr, viValue);
